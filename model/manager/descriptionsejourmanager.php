<?php 
 require("model/classes/descriptionsejourclass.php");
 ...
 class DescriptionSejourManager{
    ...
    public function updateDescrpiptionSejour(Descriptionsejour $uneDescriptionSejour){
        try {
            ...
            $sql =$connex->prepare("UPDATE sejour set titre=:titre, image=:image,titreImage=:titreImage,texte=:texte, numOrdre=:numOrdre,idSejour=:idSejour WHERE idDescription=:id ");
    ...
    public function getDescriptionSejourByIdSejour($idSejour)
    {
        try {
            $connex=$this->lePDO;
            $sql =$connex->prepare("SELECT * FROM descriptionsejour WHERE idSejour =:uneId ORDER BY numOrdre ");
            $sql->bindParam(":uneId", $idSejour);
            $sql->execute();
            $sql->setFetchMode(PDO::FETCH_CLASS,"Descriptionsejour");
    ...
    public function deleteReservationById($idDescription){
        try {
            $connex=$this->lePDO;
